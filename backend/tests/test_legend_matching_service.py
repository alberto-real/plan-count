...
def test_exact_color_and_style_match():
    ...
    legend = [
        LegendEntry(
            key="R1", label="Wall A", colorHex="#ff0000", linetype="CONTINUOUS", lineweight=25, isDashed=False
        )
    ]
    ...
    assert [m.model_dump() for m in matched] == [
        {"key": "R1", "label": "Wall A", "colorHex": "#ff0000", "linearMeters": 10.0, "isDashed": False}
    ]
...
def test_no_match_when_color_outside_tolerance():
    ...
    assert [m.model_dump() for m in matched] == []
    assert [u.model_dump() for u in undetermined] == [
        {"colorHex": "#00ff00", "linetype": "CONTINUOUS", "lineweight": 25, "linearMeters": 5.0, "isDashed": False}
    ]
...
def test_same_color_different_linetype_stays_separate():
    ...
    legend = [
        LegendEntry(key="R3", label="Trasdossat", colorHex="#0000ff", linetype="CONTINUOUS", lineweight=25, isDashed=False),
        LegendEntry(key="R3*", label="Trasdossat humit", colorHex="#0000ff", linetype="DASHED2", lineweight=25, isDashed=True),
    ]
    matched, undetermined = match_geometry_to_legend(style_groups, legend, color_tolerance=30.0)
...
def test_matched_entry_isdashed_reflects_the_matched_legend_entry():
    style_groups = {("#0000ff", "DASHED2", 25): 20.275}
    legend = [
        LegendEntry(
            key="R3*", label="Trasdossat humit", colorHex="#0000ff", linetype="DASHED2", lineweight=25, isDashed=True
        )
    ]
    matched, _undetermined = match_geometry_to_legend(style_groups, legend, color_tolerance=30.0)
    assert matched[0].isDashed is True
...
def test_no_match_when_linetype_differs_despite_identical_color():
    ...
    legend = [
        LegendEntry(
            key="R3", label="Trasdossat", colorHex="#0000ff", linetype="CONTINUOUS", lineweight=25, isDashed=False
        )
    ]
    matched, undetermined = match_geometry_to_legend(style_groups, legend, color_tolerance=30.0)
    assert [m.model_dump() for m in matched] == []
...
def test_matches_when_lineweight_differs_but_linetype_and_color_match():
    """Real DXF exports commonly have a legend row's lineweight resolved
    differently from the same-material geometry's lineweight (e.g. one
    BYLAYER-resolved on a different layer) — lineweight must not block an
    otherwise-correct match."""
    style_groups = {("#ff0000", "CONTINUOUS", 13): 7.5}
    legend = [
        LegendEntry(
            key="R1", label="Wall A", colorHex="#ff0000", linetype="CONTINUOUS", lineweight=25, isDashed=False
        )
    ...
    matched, undetermined = match_geometry_to_legend(style_groups, legend, color_tolerance=30.0)
    assert [m.model_dump() for m in matched] == [
        {"key": "R1", "label": "Wall A", "colorHex": "#ff0000", "linearMeters": 7.5, "isDashed": False}
    ]
    assert [u.model_dump() for u in undetermined] == []
